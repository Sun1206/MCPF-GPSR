...
class CLIP_3D(object):
    ...
    def proceed(self):
        ...
        def collect_predictions(loader):
            ...
            with torch.no_grad():
                for batch_idx, (ts_x, od_x, com_x, com_y) in tqdm(enumerate(loader)):
                    if self.multi_gpu:
                        od_input = od_x.reshape(od_x.size(0), od_x.size(1)*od_x.size(2), -1).to(dtype=torch.bfloat16, device=self.device)
                        ts_input = ts_x.reshape(ts_x.size(0), ts_x.size(1)*ts_x.size(2), -1).permute(0, 2, 1).to(self.device)
                    ...
                    targets.append(com_y.reshape(com_y.size(0), com_y.size(1)*com_y.size(2), -1).cpu().numpy())
                    inputs.append(com_x.reshape(com_x.size(0), com_x.size(1)*com_x.size(2), -1).cpu().numpy())
            return (
        ...
        val_predictions, val_targets, val_inputs = collect_predictions(valid_loader)
        all_predictions, test_targets_from_loader, test_inputs = collect_predictions(test_loader)
        save_pred_dir = os.environ.get("BINTS_SAVE_PRED_DIR")
        if save_pred_dir:
            os.makedirs(save_pred_dir, exist_ok=True)
            np.savez_compressed(
                os.path.join(save_pred_dir, f"{self.dataset}_seq{self.seq_day}_pred{self.pred_day}_khop{self.khop}.npz"),
                val_pred=val_predictions,
                val_y=val_targets,
                val_x=val_inputs,
                test_pred=all_predictions,
                test_y=test_targets_from_loader,
                test_x=test_inputs,
            )
        if False:
